...
[![Build Status](https://travis-ci.com/TGSAN/go-tun2socks.svg?branch=master)](https://travis-ci.com/TGSAN/go-tun2socks)
...
```sh
go get github.com/TGSAN/go-tun2socks
cd $GOPATH/src/github.com/TGSAN/go-tun2socks
go get -d ./...
...
The core part of this project is the `core` package, it focuses on `tun2socks`'s `2` part, the core package has fully IPv4/IPv6, TCP/UDP support, and only depends on lwIP (including a few [platform-dependent code](https://github.com/TGSAN/go-tun2socks/tree/master/core/src/custom)) and Go's standard library. On the one hand, IP packets input to or output from the `lwIP Stack` that initialized by `core.NewLWIPStack()`, on the other hand, TCP/UDP connections would "socksified" by the core package and can be handled by your own `core.TCPConnHandler`/`core.UDPConnHandler` implementation.
...
https://github.com/TGSAN/go-tun2socks-ios

## Creating an AAR library for Android

https://github.com/TGSAN/go-tun2socks-android
